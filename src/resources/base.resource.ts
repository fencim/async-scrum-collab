...
export abstract class BaseResource<T extends IBaseResourceModel> {
  ...
  private async normalizeDocKey(key: string, data: T) {
    ...
    const isKeyFieldSet =
      !!this.keyField && genKey === record[this.keyField];
  ...
  private async syncPostDoc(doc: IDocStore<T>) {
    ...
    if (/^(saved|createError)$/.test(doc.status)) {
      ...
      /^(updated|updateError|patched|patchError)$/.test(doc.status)
    ) {
  ...
  private shouldSyncOn(docStatus: DocStatus) {
    const shouldSync =
      this.syncingOn === true ||
      (this.syncingOn instanceof Array &&
        this.syncingOn.includes(docStatus));
    if (
      !shouldSync &&
      docStatus != 'synced' &&
  ...
  private isSame(a: T, b: T) {
    ...
    return propsA.reduce(
      (same, prop) =>
        same && typeof a[prop] != 'object' && a[prop] === b[prop],
      true
    );
  }
  ...
  protected async syncFromOnline(
    ...
    const mergedDocs = offlineDocs.filter((offlineDoc) => {
      const matchedRecordOnline = onlineResult.find(
        (onlineRecord) =>
          this.getKeyOf(onlineRecord) ===
          this.getKeyOf(offlineDoc.data) ||
      ...
      (onlineRecord) =>
        !mergedDocs.find(
          (mergeRecord) =>
            ...
            this.getKeyOf(mergeRecord.data) ==
            this.getKeyOf(onlineRecord)
        )
    );
    ...
    const onlineDeletedDocs = offlineDocs.filter(
      (offlineDoc) =>
        (offlineDoc.status !== 'synced' ||
          !mergedDocs.find(
            (mergeDoc) => mergeDoc.key === offlineDoc.key
          )) &&
  ...
  protected matchesFilter(a: T, filters?: Filters) {
    ...
    if (!Array.isArray(f) && typeof f == 'object') {
      ...
      const filterIndex = 1;
      const isPegeable =
        Array.isArray(f) && typeof f[0] == 'object' && !!((f[0] as unknown as { rowsPerPage: number }).rowsPerPage);
      return this.filterCb(a, isPegeable ? f[filterIndex] as unknown as string[] : f);
    }
    return true;
  }
  ...
  protected getKeyOf(v: T): string {
    if (typeof v == 'object') {
      if (
        ...
        return (v as unknown as { [key: string]: string })[
          this.keyField
        ];
      } else if (
        this.altKey &&
        ...
        return this.hashName(JSON.stringify(v))
          .toString()
          .replace('-', 'N');
      }
    } else {
    ...
    if (this.getAllCb) {
      ...
      if (this.shouldSyncOn('synced')) {
        if (typeof syncCb == 'function') {
          ...
          if (onlineResult) {
            return await this.syncFromOnline(
              result,
              onlineResult,
              filters
            ) || result;
  ...
  subscribeOn(
    ...
    const subject = `${this.entity}:${typeof docOrKey == 'string' ? docOrKey : docOrKey.key
      }`;
  ...
  async updatePropertiesFrom(
    ...
    for (const prop of props) {
      ...
      (existing.data as unknown as Record<string, string>)['*' + String(prop)] = oldData as unknown as string;
      existing.data[prop] = (
    ...
    if (diff > 0) {
      ...
      const status =
        /^(synced|updated|patched|updateError|patchError)$/.test(
          existing.status
        )
          ? 'patched'
          : 'saved';
  ...
  filterToStr(filters?: Filters<Entity> | undefined) {
    ...
    return Object.keys(f).reduce(
      (p, c, i) =>
        p +
        `${i > 0 ? ';' : ''}${c}:${(f as Record<string, string>)[c]} `,
      ''
    );
  }
  streamDocWith(
    ...
    const online = onlineObservable
      .pipe(
        switchMap(async (list) => {
          if (offlineDocs) {
            const updated = await this.syncFromOnline(
              offlineDocs,
              list,
              filters
            );
